...
const addNewStock = async (req, res, next) => {
  ...
  try {
    if (!isValidObjectId(value.productId)) {
      return res
        .status(400)
        .json({ error: 'Invalid productId: IDs must be valid' });
    }
    ...
    if (!productExists) {
      return res
        .status(400)
        .json({ error: 'Invalid productId: Product does not exist' });
    }
  ...
  try {
    ...
    if (!stocks || stocks.length === 0) {
      return res.status(404).json({
        message: `No stocks found for product with _id: ${productId}`,
      });
    }
    res.status(200).json(stocks);
  } catch (error) {
...
const updateStock = async (req, res, next) => {
  ...
  try {
    ...
    if (!stock) {
      return res
        .status(404)
        .json({ message: `Stock with _id: ${stockId} not found` });
    }
    ...
    const stockUpdate = await Stock.findByIdAndUpdate(stockId, value, {
      new: true,
    });
